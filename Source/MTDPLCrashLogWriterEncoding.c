...
#include "MTDPLCrashLogWriterEncoding.h"

#define MAX_UINT64_ENCODED_SIZE 10

/* --- wire format enums --- */
typedef enum {
        MTDPLPROTOBUF_C_WIRE_TYPE_VARINT,
        MTDPLPROTOBUF_C_WIRE_TYPE_64BIT,
        MTDPLPROTOBUF_C_WIRE_TYPE_LENGTH_PREFIXED,
        MTDPLPROTOBUF_C_WIRE_TYPE_START_GROUP,     /* unsupported */
        MTDPLPROTOBUF_C_WIRE_TYPE_END_GROUP,       /* unsupported */
        MTDPLPROTOBUF_C_WIRE_TYPE_32BIT
} MTDPLProtobufCWireType;
...
size_t plcrash_writer_pack (plcrash_async_file_t *file, uint32_t field_id, MTDPLProtobufCType field_type, const void *value) {
    ...
    switch (field_type)
    {
        case MTDPLPROTOBUF_C_TYPE_SINT32:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_VARINT;
            rv += sint32_pack (*(const int32_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_INT32:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_VARINT;
            rv += int32_pack (*(const uint32_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_UINT32:
        case MTDPLPROTOBUF_C_TYPE_ENUM:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_VARINT;
            rv += uint32_pack (*(const uint32_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_SINT64:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_VARINT;
            rv += sint64_pack (*(const int64_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_INT64:
        case MTDPLPROTOBUF_C_TYPE_UINT64:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_VARINT;
            rv += uint64_pack (*(const uint64_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_SFIXED32:
        case MTDPLPROTOBUF_C_TYPE_FIXED32:
        case MTDPLPROTOBUF_C_TYPE_FLOAT:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_32BIT;
            rv += fixed32_pack (*(const uint32_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_SFIXED64:
        case MTDPLPROTOBUF_C_TYPE_FIXED64:
        case MTDPLPROTOBUF_C_TYPE_DOUBLE:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_64BIT;
            rv += fixed64_pack (*(const uint64_t *) value, scratch + rv);
            if (file != NULL)
                plcrash_async_file_write(file, scratch, rv);
            break;
        case MTDPLPROTOBUF_C_TYPE_BOOL:
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_VARINT;
        ...
        case MTDPLPROTOBUF_C_TYPE_STRING:
        {
            uint32_t sublen = (uint32_t) strlen (value);
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_LENGTH_PREFIXED;
        ...
        case MTDPLPROTOBUF_C_TYPE_BYTES:
        {
            const MTDPLProtobufCBinaryData * bd = ((const MTDPLProtobufCBinaryData*) value);
        ...
        case MTDPLPROTOBUF_C_TYPE_MESSAGE:
        {
            scratch[0] |= MTDPLPROTOBUF_C_WIRE_TYPE_LENGTH_PREFIXED;
            rv += uint32_pack (*(const uint32_t *) value, scratch + rv);
            if (file != NULL)
        ...
        default:
            MTDPLCF_DEBUG("Unhandled field type %d", field_type);
            abort();
    }
    return rv;
}
